...
func Authenticate() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Get current subject
		sub, err := components.GlobalCache.Get(sessionId)
		if errors.Is(err, bigcache.ErrEntryNotFound) {
			c.AbortWithStatusJSON(401, components.RestResponse{Message: "user hasn't logged in yet"})
			return
		}
...
func Authorize(obj string, act string, adapter persist.Adapter) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			log.Println(err)
			c.AbortWithStatusJSON(500, components.RestResponse{Message: "error occurred when authorizing user"})
			return
		}
		if !ok {
			c.AbortWithStatusJSON(403, components.RestResponse{Message: "forbidden"})
			return
		}
		c.Next()
	}
