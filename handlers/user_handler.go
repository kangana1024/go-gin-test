...
func Login(c *gin.Context) {
	...
	for iter := components.GlobalCache.Iterator(); iter.SetNext(); {
		info, err := iter.Value()
		if err != nil {
			continue
		}
		if string(info.Value()) == username {
			components.GlobalCache.Delete(info.Key())
	...
	if username == "alice" && password == "111" {
		...
		c.JSON(200, components.RestResponse{Message: "no such account"})
		return
	}
	...
	err = components.GlobalCache.Set(sessionId, []byte(username))
	if err != nil {
	...
	c.JSON(200, components.RestResponse{Code: 1, Message: username + " logged in successfully"})
}
